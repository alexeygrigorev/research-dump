...
def check_live_configuration() -> None:
    ...
    for name in ('privacy', 'terms', 'imprint'):
...
def enforce_rate_limit(environ: dict) -> None:
    ...
    key = hmac.new(WEBHOOK_SECRET.encode(), address.encode(), hashlib.sha256).hexdigest()
    with closing(connect()) as con, con:
        con.execute('BEGIN IMMEDIATE')
        con.execute('DELETE FROM rate_limits WHERE received_at < ?', (now - 900,))
...
def save_application(environ: dict) -> dict:
    ...
    with closing(connect()) as con, con:
        con.execute('BEGIN IMMEDIATE')
        old = con.execute('SELECT id,payload_hash FROM applications WHERE submission_key=?', (key,)).fetchone()
        if old:
            ...
            con.execute('INSERT INTO applications (id,submission_key,payload_hash,created_at,answers,terms_version) VALUES (?,?,?,?,?,?)',
                        (application_id, key, digest, int(time.time()), payload, TERMS_VERSION))
...
def handle_webhook(environ: dict) -> dict:
    ...
    with closing(connect()) as con, con:
        con.execute('BEGIN IMMEDIATE')
        if con.execute('SELECT 1 FROM stripe_events WHERE event_id=?', (event['id'],)).fetchone():
            return {'received': True, 'duplicate': True}
        if session.get('payment_link') != PAYMENT_LINK_ID:
            # Unrelated account payments never mark an application as paid.
            con.execute('INSERT INTO stripe_events VALUES (?,?,?)', (event['id'], kind, now))
            return {'received': True, 'ignored': True}
        reference = session.get('client_reference_id')
        row = con.execute('SELECT id FROM applications WHERE id=?', (reference,)).fetchone()
        ...
        flags = []
        if not application_id: flags.append('unmatched_application')
        if not valid_price: flags.append('unexpected_amount_or_currency')
        event_created = event.get('created')
        if isinstance(event_created, (int, float)) and event_created > CLOSES:
            flags.append('payment_after_application_deadline')
        if application_id and con.execute("SELECT 1 FROM payments WHERE application_id=? AND session_id<>? AND payment_status='paid'", (application_id, session_id)).fetchone():
            flags.append('possible_duplicate_payment')
        previous = con.execute('SELECT payment_status FROM payments WHERE session_id=?', (session_id,)).fetchone()
        # A delayed unpaid/failed event must never overwrite a recorded paid result.
        if previous and previous['payment_status'] == 'paid': status = 'paid'
        intent = session.get('payment_intent')
        if isinstance(intent, dict): intent = intent.get('id')
        con.execute('''INSERT INTO payments VALUES (?,?,?,?,?,?,?,?)
                       ON CONFLICT(session_id) DO UPDATE SET
                       payment_status=excluded.payment_status,flag=excluded.flag''',
                    (session_id, application_id, intent, status, amount, currency, now, ','.join(flags)))
        con.execute('INSERT INTO stripe_events VALUES (?,?,?)', (event['id'], kind, now))
...
def application(environ, start_response):
    try:
        ...
        if method == 'GET' and path in ('/', '/index.html'):
            ...
            if MODE == 'live':
                config = dict(mode='live', applicationEndpoint='/api/applications',
                              ...
                              imprintUrl='/legal/imprint', contactEmail=CONTACT,
        ...
        if method == 'GET' and path in {'/legal/privacy', '/legal/terms', '/legal/imprint'}:
            content = (ROOT / 'legal' / (path.rsplit('/', 1)[1] + '.html')).read_bytes()
            return respond(start_response, 200, content, 'text/html; charset=utf-8')
        if method == 'GET' and path == '/health':
            return respond(start_response, 200, b'{"ok":true}')
...
if __name__ == '__main__':
    ...
    print('Local preview only: http://localhost:8080 — not a public production server.')
